...
export class AnalyticsService {
  ...
  getUserFavouriteBoards(user: string, limit: number): Promise<ResGetUserFavouriteBoards> {
    return this.userActionModel
      .aggregate([
        { $match: { user, 'data.targetType': 'board' } },
        {
          $group: {
  ...
  getUserFavouriteTasks(user: string, limit: number): Promise<ResGetUserFavouriteTasks> {
    return this.userActionModel
      .aggregate([
        { $match: { user, 'data.targetType': 'task' } },
        {
          $group: {
            _id: { id: '$data.targetId' },
            count: { $sum: 1 },
        ...
        { $sort: { count: -1 } },
        { $limit: limit },
        { $project: { task: '$_id.id', _id: 0, count: 1 } },
      ])
      .exec();
  }
}
